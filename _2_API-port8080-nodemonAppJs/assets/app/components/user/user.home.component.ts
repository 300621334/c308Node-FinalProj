...
export class UserHomeComponent {
    ...
    doInit() {
        ...
        this.userhomeservice.getBooks(this.studentid).subscribe(
            val => {
                ...
                for (let bk of val.data) {
                    this._books.push(new Book(bk.title, bk.author,bk.section, bk.isbn));
                }
            },
        ...
        this.userhomeservice.getAvailableBooks().subscribe(
            val => {
                ...
                for (let bk of val.data) {
                    this._availablebooks.push(new Book(bk.title, bk.author, bk.section, bk.isbn));
                }
            },
            error => { console.error(error) }
        );
    ...
    onRemove(isbn) {
        this._books.forEach((item, index) => {
            if (item.isbn === isbn) {
                this._availablebooks.push(new Book(item.title, item.author,item.section, item.isbn));
    ...
    onAdd(isbn) {
        this._availablebooks.forEach((item, index) => {
            if (item.isbn === isbn) {
                this._books.push(new Book(item.title, item.author,item.section, item.isbn));
